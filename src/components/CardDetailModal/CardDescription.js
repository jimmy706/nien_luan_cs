...
import { IconButton, PrimaryButton, TooltipHost, Icon} from "office-ui-fabric-react";
...
function CardDescription(props) {
    ...
    async function handleSubmitDesc(e) {
        ...
        const updateResult = await cardAPIs.updateCardDescription(cardId,desc,token);
    ...
    return (
        <div className="card-desc section-wrapper">
            ...
            </h4>
            {
                !openForm && (
                    desc.trim()
                        ? <div onClick={handleOpenForm} className="desc-display" dangerouslySetInnerHTML={{__html: html}}/>
                        : <div onClick={handleOpenForm} className="empty-desc">Add a more detail description...</div>
                )
            }

            <form style={{display: `${openForm ? 'block' : 'none'}`}} onSubmit={handleSubmitDesc}>
                <TextareaAutosize
